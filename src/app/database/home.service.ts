...
evenements=new BehaviorSubject([])
  constructor(private plat: Platform, /*private sqliteporter:SQLitePorter ,*/private sqlite:SQLite,private http:HttpClient)
   ...
   loadevenements()
   {
    return this.db.executeSql('select * from evenements',[]).then(data=>{
      let evenements: any[] = [];
      if(data.rows.length>0)
      {
        for(var i=0;i<data.rows.length;i++)
        {
         ...
         description:data.rows.item(i).description,
         title:data.rows.item(i).title,
         x:data.rows.item(i).place,
         })
        }
      }
      this.evenements.next(evenements);
    })
   }
   ...
   {
     return this.evenements.asObservable()
   }
